...
- **Commit area (S-C):** draft state → Review & commit + Discard + "Draft" indicator; clean state → "No changes to commit" + Record payment; both reuse the existing dialogs/actions with identical behavior, `expectedVersion`, and ownership gating. No footer bar, no header CTA.
