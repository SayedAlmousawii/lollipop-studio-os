...
- **Finishing = live write, NOT OrderCommit-staged (decided 2026-06-13).** Note this diverges from session configurations, which stage even operational/no-price selections through OrderCommit as zero-impact "metadata" snapshot lines. Album finishing instead writes immediately via `updateOrderAlbumFinishing` (F1 / Spec 169) because finishing is **production metadata** refined across the order lifecycle (often post-commit), not a sale-time pricing decision — forcing a stage→commit ceremony for a thread-color change on a settled order would be worse. **Rationale, so it isn't re-litigated:** OrderAlbum is intentionally a separate operational entity *outside* the OrderCommit snapshot (which has no album line kind; albums reconcile post-commit via `syncOrderAlbumsAfterCommit`); routing finishing through staging would require adding album state to the snapshot/draft (new staging domain + reducer + reconciliation) — reopening the engine. **Parked, not adopted:** if an audit trail for finishing is later wanted, emit an activity/timeline entry on each finishing write (NOTE_ADDED-style) — keeps the audit without staging. Revisit staged-metadata only if real usage shows finishing is genuinely sale-time / mid-draft and must commit atomically with pricing. Permission note: the finishing action is gated on `ORDER_FINANCIAL_UPDATE` because that's the de-facto "can edit this order's Sales surface" permission (no operational-edit permission exists); add a code comment so it doesn't read as a copy-paste bug.
