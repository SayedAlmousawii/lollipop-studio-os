...
- OrderCommit execution is additive so far: `OrderCommitDocument` is invoice-only, `committedFromDraftVersion` is the future commit concurrency guard, and the Task 2 materializer is transaction-client-only with no financial mapper, document writer, commit orchestration, POS routing, or invoice/payment/refund mutation wired yet.
