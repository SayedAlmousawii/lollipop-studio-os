...
- Feature 57b: POS package composition area — `/orders/[orderId]/sales` now renders structured package deliverable cards, visible raw total/bundle adjustment/package price lines, package upgrade picker, same-category item upgrade picker, locked-invoice blocking, and service-layer mutations that write order package changes or product-backed upgrade delta add-ons without mutating package templates. Files changed: `app/orders/[orderId]/sales/page.tsx`, `app/orders/[orderId]/sales/actions.ts`, `src/components/orders/pos-package-composition.tsx`, `src/modules/orders/order.schema.ts`, `src/modules/orders/order.service.ts`. Assumption: the 57b "Replace" action uses the same same-category upgrade picker because replacement pricing is specified as the item upgrade delta in this unit.
